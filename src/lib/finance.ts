/**
 * Frontend live-preview mirror of the authoritative server calculation
 * (`server/src/domain/finance.ts`). The two implementations are kept identical
 * and are pinned together by a shared set of golden vectors
 * (`server/tests/unit/finance.vectors.json`, asserted by `finance.test.ts`).
 *
 * All money is integer paise. Rates are basis points (100 bps = 1%).
 * Never render a figure that was not computed from the actual inputs.
 */
export type MoratoriumInterestHandling = 'serviced' | 'capitalised';
...
export function calculateFinancePlan(input: FinanceInput): FinancePlan {
  ...
  const { projectCostPaise, ownContributionPaise, requestedLoanPaise, moratoriumMonths, moratoriumInterestHandling } = input;
  ...
  const repaymentMonths = input.tenureIncludesMoratorium ? input.tenureMonths - moratoriumMonths : input.tenureMonths;
  ...
  const totalRepaymentPaise =
    schedule.reduce((s, r) => s + r.paymentPaise, 0) + moratoriumSchedule.reduce((s, r) => s + r.paymentPaise, 0);
...
export const paiseToRupees = (p: number): number => p / RUPEE;
export const rupeesToPaise = (r: number): number => roundPaise(r * RUPEE);
...
export function formatPaise(paise: number | null | undefined, opts?: { paise?: boolean }): string {
  if (paise == null) return '—';
  return (opts?.paise ? inrFmt2 : inrFmt).format(paiseToRupees(paise));
}
